class Walker
{
  ...
  // the function that applies the gravitational attraction between walkers
  public PVector calculateGravitationalAttraction(Walker walker)
  {
    // calculate the direction and distance between this and another walker
    PVector direction = PVector.sub(this.position, walker.position); 
    float distance = direction.mag();
    direction.normalize(); 
    
    // constrain distance within a range
    distance = constrain(distance, 5, 25);
    
    // calculate the force of the gravitational attraction
    float gravitationalStrength = (this.gravitationalConstant * this.mass * walker.mass / (distance * distance));
    PVector force = direction.mult(gravitationalStrength);
    return force;
  }
}
